...
// ========== BACKUP CAROUSEL DATA (Buyer-focused) ==========
...
export default function BuyerDashboard() {
  ...
  const startAutoScroll = () => {
    const interval = setInterval(() => {
      setActiveSlide((prev) => {
        ...
        carouselRef.current?.scrollTo({ x: next * screenDimensions.width, animated: true });
        return next;
      });
    }, 6000);
  ...
  const fetchCarouselItems = async () => {
    try {
      const carouselUrl = `${API_BASE_URL}/api/carousel/items/`;
      console.log("🎠 Fetching carousel items from:", carouselUrl);
      
      // Try carousel API endpoint (correct path: /api/carousel/items/)
      ...
      if (response.ok) {
        ...
        console.log("📡 Carousel API Response Data:", JSON.stringify(items, null, 2));
        ...
        console.log(`⚠️ API returned status: ${response.status}, body: ${errorText.substring(0, 200)}`);
      }
      ...
      if (token) {
        try {
          ...
          if (allEvents && Array.isArray(allEvents) && allEvents.length > 0) {
            ...
            console.log(`✅ Loaded ${sortedEvents.length} events with auth`);
            return;
          }
        } catch (error) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (allEvents && Array.isArray(allEvents) && allEvents.length > 0) {
            const sortedEvents = allEvents.sort((a, b) => b.id - a.id);
            setEvents(sortedEvents);
            console.log(`✅ Loaded ${sortedEvents.length} events without auth`);
            return;
          }
        }
      } catch (publicError) {
  ...
  const cardWidth = screenDimensions.width - 40;
  const horizontalPadding = Math.min(20, screenDimensions.width * 0.05);
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <View style={styles.carouselSection}>
          {loadingCarousel ? (
            <View style={[styles.carouselLoader, { width: cardWidth }]}>
            ...
            <>
              <ScrollView
                ...
                onMomentumScrollEnd={(e) => {
                  const slide = Math.round(e.nativeEvent.contentOffset.x / screenDimensions.width);
                  setActiveSlide(slide);
                }}
              >
                {carouselData.map((item) => (
                  <View key={item.id} style={{ width: screenDimensions.width, alignItems: 'center' }}>
                      ...
                      <View style={styles.cardContent}>
                        <View style={styles.cardHeader}>
                          <View style={styles.iconCircle}>
                            <Ionicons 
                              name={(item.icon && item.icon !== 'none' ? item.icon : 'cube-outline') as any} 
                              size={24} 
                              color={item.color} 
                            />
                          </View>
                          <Text style={styles.cardTitle}>{item.title || 'Featured'}</Text>
                        </View>
        ...
        <View style={[styles.sectionContainer, { paddingHorizontal: horizontalPadding }]}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{t.buyer.upcomingEvents}</Text>
            <TouchableOpacity onPress={() => router.push('/events-list')}>
              <Text style={styles.seeAllText}>{t.buyer.seeAll}</Text>
            </TouchableOpacity>
          </View>
          ...
          {loadingEvents ? (
            ...
            <View style={styles.emptyStateContainer}>
               ...
               <Text style={styles.emptyStateText}>{t.buyer.noEventsScheduled}</Text>
               <Text style={styles.emptyStateSubtext}>{t.buyer.checkBackLater}</Text>
            </View>
          ) : (
                  ...
                  {(event.is_cancelled || isPast) && (
                    <View style={[
                      ...
                      <Text style={[
                        ...
                        {event.is_cancelled ? 'Cancelled' : 'Ended'}
                      </Text>
                    </View>
                  )}
        ...
        <View style={[styles.sectionContainer, { paddingHorizontal: horizontalPadding }]}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{t.buyer.latestNews}</Text>
            {loadingUpdates && <ActivityIndicator size="small" color="#1565C0" />}
          </View>
              ...
              >
                {update.imageUrl ? (
                  ...
                  <View style={[styles.updateImagePlaceholder, {
                    backgroundColor: update.category === 'Policy' ? '#E3F2FD' : 
                                   update.category === 'Market' ? '#FFF8E1' : 
                                   update.category === 'Weather' ? '#E1F5FE' : '#F5F5F5'
                  }]}>
                    <Ionicons 
                      name={
                      ...
                      size={32} 
                      color={
                        update.category === 'Policy' ? '#1565C0' : 
                        update.category === 'Market' ? '#F57F17' : 
                        update.category === 'Weather' ? '#0277BD' : '#757575'
                      }
                    />
                  </View>
                )}
                <View style={styles.updateTextContainer}>
                  <View style={styles.updateHeader}>
                    <View style={[styles.categoryBadge, { 
                      backgroundColor: update.category === 'Policy' ? '#1565C0' : 
                                     update.category === 'Market' ? '#F57F17' : 
                                     update.category === 'Weather' ? '#0277BD' : '#757575' 
                    }]}>
                  ...
                  {update.url && (
                    <View style={styles.readMoreContainer}>
                      <Text style={styles.readMoreText}>{t.buyer.readMore}</Text>
...
const styles = StyleSheet.create({
  ...
  // Carousel Styles
  carouselSection: { marginTop: 0, marginBottom: 10 },
  carouselLoader: { 
    height: 180, 
